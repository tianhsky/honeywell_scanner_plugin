...
public class Decoder {
  ...
  static {
    ...
    //TODO
    // Dim DAR As String ' Class
    // Dim DAS As String ' Restrictions
    // Dim DAT As String ' Endorsements
    // Dim DAW As String ' Weight
    // Dim DAZ As String ' Hair Color
    // Dim DBH As String ' Organ Donor
    // Dim DCA As String ' Georgia License Class
    // Dim DCB As String ' Georgia Driver's License Only Restrictions
    // Dim DCD As String ' Georgia Driver's License Only Endorsements
  ...
  public int getIdentificationNumber() {
    return (Integer) getHeaders().get("IdentificationNumber");
  }

  public int getVersionNumber() {
    return (Integer) getHeaders().get("VersionNumber");
  }

  public int getJurisdictionVerstion() {
    return (Integer) getHeaders().get("JurisdictionVerstion");
  }
  ...
  public int getSubfileOffset() {
    return (Integer) getHeaders().get("SubfileOffset");
  }

  public int getSubfileLength() {
    return (Integer) getHeaders().get("SubfileLength");
  }

  protected HashMap<String, Object> decodeHeaders(String data) {
    ...
    if (versionNumber <= 1) {
      ...
      length = Integer.parseInt(data.substring(25, 27));
    } else {
  ...
  protected HashMap<String, String> decodeSubFile(String data) {
    ...
    // String subfile = getSubfile(data);
    ...
    for (String l : lines) {
      ...
      if (!l.startsWith("D")) {
        if (l.startsWith("ANSI")) {
          l = l.substring(l.lastIndexOf("DL") + 2);
        } else if (l.indexOf("DAQ") != -1) {
          l = l.substring(l.lastIndexOf("DAQ"));
        } else
          continue;
      }
      if (l.length() > 3) {
        ...
        if (fields.get(key) != null) {
          hm.put(fields.get(key), value.replace("@", ","));
        }
      }
    }
    return hm;
  ...
  private static String getSubfile(String data) {
    int subFileStart = 2;
    while (subFileStart-- > 0) {
